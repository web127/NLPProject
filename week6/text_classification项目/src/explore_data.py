...
def analyze_label_distribution(data, id2name, split_name, output_dir: Path):
    ...
    plt.tight_layout()
    save_path = output_dir / f"a_label_dist_{split_name}.png"
...
def analyze_char_length(data, id2name, split_name, output_dir: Path):
    ...
    plt.tight_layout()
    save_path = output_dir / f"a_char_length_{split_name}.png"
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f"  图表已保存 → {save_path}")
...
def analyze_length_by_label(data, id2name, split_name, output_dir: Path):
    ...
    plt.tight_layout()
    save_path = output_dir / f"a_length_by_label_{split_name}.png"
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f"  图表已保存 → {save_path}")
...
def analyze_token_length(data, tokenizer, split_name, output_dir: Path):
    ...
    plt.tight_layout()
    save_path = output_dir / f"a_token_length_{split_name}.png"
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f"  图表已保存 → {save_path}")
