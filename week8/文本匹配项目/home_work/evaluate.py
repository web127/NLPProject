...
这个文件的核心任务是：
  “模型训练完以后，怎样客观判断它效果好不好？”

初学者阅读建议：
  1. 先看 main()，理解评估总流程
  2. 再看 eval_biencoder()，理解为什么需要阈值搜索
  3. 再看 eval_crossencoder()，理解它为什么和普通分类几乎一样
  4. 最后看 plot_similarity_distribution()，理解可视化怎么帮助分析模型
...
def eval_biencoder(model, loader, device, find_threshold=True, threshold=0.5):
    ...
    为什么 BiEncoder 需要阈值？
      因为它输出的不是“类别”，而是一个连续分数，比如：
        0.91 -> 很像
        0.63 -> 有点像
        0.12 -> 不像
      所以必须设一个分界线 threshold，例如 0.55：
        >= 0.55 判为相似
        <  0.55 判为不相似
    """
    model.eval()
...
def eval_crossencoder(model, loader, device):
    ...
    CrossEncoder 为什么更像普通分类？
      因为它最后直接输出两个类别的 logits：
        [不相似分数, 相似分数]
      然后只要取最大值所在类别即可。
    """
    model.eval()
...
def main():
    ...
    print("提示：若是第一次实验，建议优先评估 validation，而不是 test。")
    ...
    if args.model_type == "biencoder":
        ...
        # 这里从 checkpoint 中恢复训练时的关键参数，
        # 避免“训练时用 mean pooling，评估时却误用 cls pooling”这种错误。
